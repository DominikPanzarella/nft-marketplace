...
import { RangeFilter, RangeValue } from "@/types/filter-types";
...
const PriceSliderPip: React.FC<PriceSliderPipProps> = ({ animation = "height", isInRange }) => {
  ...
  // This is a solution to fix warning: hydratation server and client mismatch
  const [height, setHeight] = React.useState("30%");

  React.useEffect(() => {
    const rand = Math.floor(Math.random() * 100);

    setHeight(clampValue(rand, 30, 100) + "%");
  }, []);
...
const PriceSlider = React.forwardRef<HTMLDivElement, PriceSliderProps>(
  ({ range, animation, className, ...props }, ref) => {
    ...
    const debouncedOnChangeEnd = useDebounceCallback(
      (newValue: RangeValue) => {
    ...
    const onMinInputValueChange = React.useCallback(
      (inputValue: string) => {
        ...
        if (!isNaN(newValue)) {
          ...
          debouncedOnChangeEnd([clampedValue, value[1]]);
        }
      },
    ...
    const onMaxInputValueChange = React.useCallback(
      (inputValue: string) => {
        ...
        if (!isNaN(newValue) && newValue <= maxValue) {
          ...
          debouncedOnChangeEnd([value[0], newValue]);
        }
      },
      ...
      <div className={cn("flex flex-col gap-3", className)}>
        ...
        <div className="flex items-center">
          <Input
            ...
            startContent={<p className="text-default-400">ETH</p>}
            type="number"
            value={`${value[0]}`}
